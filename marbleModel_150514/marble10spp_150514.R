...
commonSample <- sample(commSpp, replace = TRUE, size = 200)
medSample <- sample(medSpp, replace = TRUE, size = 150)
medLoSample <- sample(medLoSpp, replace = TRUE, size = 25)
rareSample <- sample(rareSpp, replace = TRUE, size = 25)

# combine into a vector
commSample <- c(commonSample, medSample, medLoSample, rareSample)
commIdentity <- c(rep("common", 150), rep("med", 125), 
	rep("medLo", 75), rep("rare", 50))
...
panelA <- prePlot + ggtitle("Pre-disturbance\n") +
	theme(plot.title = element_text(lineheight = .5, 
		face = "bold", size = 15)) + 
		geom_segment(aes(x = 10.5, y = 20.5, xend = 10.5, yend = 9.5), 				color = "gray", linetype = "solid", size = 0.1) + 
		geom_segment(aes(x = 0.5, y = 9.5, xend = 10.5, yend = 9.5), 					color = "gray", linetype = "solid", size = 0.1) + 
		geom_segment(aes(x = 0.5, y = 20.5, xend = 0.5, yend = 9.5), 					color = "gray", linetype = "solid", size = 0.1) + 
		geom_segment(aes(x = 0.5, y = 20.5, xend = 10.5, yend = 20.5), 				color = "gray", linetype = "solid", size = 0.1) +
		geom_text(aes(x = 5, y = 21), label = "Local", size = 3, color = 				"black")
...
sample.i <- sample(1:length(preVector), distSize, replace = FALSE)
sample.i
...
panelB <- prePlot + randOverlay +
	ggtitle("Post-disturbance\n") +
...
with(preDat, table(preVecF))
preSppCounts <- ddply(preDat, .(preVecF), summarise, freq = length(preVec))
...
preRenyi
##############
### Scale - sample 100/400 individuals (top left corner, 1st 100 individuals)
##############
subDim <- 1:10
...
#########
spp10dat <- rbind(randomDat, preDFcomplete)
